...
var fulltalkID = localStorage.getItem("fulltalkID");
if (fulltalkID) {
  $.ajax({
    url: "/new-user-update/" + fulltalkID + "",
    type: "PUT",
    success: function (data) {
      const newFulltalkID = data.fulltalkID;
      if (newFulltalkID) {
        localStorage.removeItem("fulltalkID");
        localStorage.setItem("fulltalkID", newFulltalkID);
        username = newFulltalkID;
  ...
  $.ajax({
    ...
    success: function (response) {
      ...
      localStorage.setItem("fulltalkID", response);
      username = response;
      runUser();
    },
...
function runUser() {
  let init = async () => {
    ...
    $.post("https://fulltalk.herokuapp.com/get-remote-users", { fulltalkID: fulltalkID })
      .done(function (data) {
        console.log(data);
        if (data[0]) {
  ...
  function onReceiveChannelMessageCallback(event) {
    ...
    chatTextArea.innerHTML +=
      "<div style='margin-top:2px; margin-bottom:2px;'><b>Random: </b>" +
      event.data +
      "</div>";
  }
  ...
  function fetchNextUser(remoteUser) {
    $.post(
      "https://fulltalk.herokuapp.com/get-next-user",
      { fulltalkID: fulltalkID, remoteUser: remoteUser },
      function (data) {
